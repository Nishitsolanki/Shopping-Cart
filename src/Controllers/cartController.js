...
const myCart = async function(req,res){
  ...
  //____________________________________If cart is not present___________________________________
    ...
    //_____________________________________Cart is already Present______________________________________
      ...
      if(!userCart){
        return res.status(404).send({status : false, message : "No cart found for such user"})
      }
...
const updateCart = async function(req,res){
 ...
 if(!user){
  ...
  if(!checkProduct){
      return res.status(4044).send({status : false, message : "Product doesn't exists!"})
  }
      ...
      if(cartData[i].productId == productId){
          ...
          if(removeProduct == 1){
            ...
            //_____________________Product Quantity greater than 1___________________________________________
              ...
              return res.status(200).send({ status: true, message: "Successs", data: reduceQuantity })
            }
          }
      }
...
const deleteCart =  async function(req,res){
  try{
       ...
       if (!isvalidObjectId(userId)) {
        ...
        let updated =  await cartModel.findOneAndUpdate({_id:cart._id },deletecart,{new:true})               
          return res.status(200).send({status: true, message: `cart has been deleted.`,data:updated})
