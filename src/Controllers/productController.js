...
const updateproduct = async function (req, res) {
  ...
  if (!checkprd) {
    return res.status(404).send({ status: false, message: "Product doesn't Exists with this productId!" })
  }
  if(checkprd.isDeleted == true){
    return res.status(404).send({ status: false, message: "Product is Deleted from store! You cannot update it" })
  }
...
const deleteProduct = async function (req, res) {
  try {
    ...
    if (product.isDeleted == false) { await productModel.findOneAndUpdate(
